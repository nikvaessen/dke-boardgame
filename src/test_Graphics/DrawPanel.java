...
public class DrawPanel {
	...
	public void draw(Graphics g) {
		...
		for (int x = 0; x < 11; x++) {
			g.drawString(Character.toString(alphabet[x]), (OFFSET_X + LENGTH +(x * LENGTH * 2)) - 5, OFFSET_Y - 5);
		}
		for (int y = 0; y < 11; y++) {
			g.drawString(Integer.toString(y), OFFSET_X/2 + (y * LENGTH), OFFSET_Y + (LENGTH * 2) + (y * LENGTH * 2));
		}
	}
}
