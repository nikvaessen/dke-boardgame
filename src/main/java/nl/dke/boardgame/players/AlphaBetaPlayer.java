...
public class AlphaBetaPlayer extends HexPlayer
{
    ...
    {
        if(t.equals(TileState.PLAYER1))
        {
            return TileState.PLAYER2;
        }
        else if(t.equals(TileState.PLAYER2))
        {
    ...
    public int[][] getGraph(Board board, TileState tilestate)
    {
        ...
        */
        ...
        {//edgepieces from left to right--RED
            for(int i = 1; i <= 11; i++)
            {
        ...
        {//edgepieces from top to bottom--BLUE
            for(int i = 1; i <= 11; i++)
            {
...
//            System.out.println("so score for player 2 is: " + result);
    ...
    public int getVertexValue(TileState tilestate, Board board, int row, int column)
    {
        ...
        if(tileOwner.equals(tilestate))
        {
            return 0; //if the tile is of the player who is evaluating the board
        }
        else if(tileOwner.equals(TileState.NEUTRAL))
        {
            return 1; // if the tile is neutral
        }
        else
        {
            return 999; //if tile is of opponent
        }
    }
}
